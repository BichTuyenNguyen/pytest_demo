...
class BasePage:
    ...
    # ==========================wait elements===============================================
    def wait_for_page_ready(self, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        wait.until(lambda driver: self.driver.execute_script("return document.readyState == 'complete'"))

    def wait_for_visibility_of_element(self, by, selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(EC.visibility_of_element_located((by, selector)))

    def wait_for_visibility_of_element_by_id(self, selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        return self.wait_for_visibility_of_element(By.ID, selector, timeout)

    def wait_for_visibility_of_element_by_css(self, selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        return self.wait_for_visibility_of_element(By.CSS_SELECTOR, selector, timeout)

    def wait_for_visibility_of_element_by_xpath(self, selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        return self.wait_for_visibility_of_element(By.XPATH, selector, timeout)

    def wait_for_text_to_be_present(self, by, selector, text, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(EC.text_to_be_present_in_element((by, selector), text))

    def wait_for_element_to_click(self, by, selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
    ...
    def check_displayed(self, by, selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        try:
    ...
    def wait_element_exist(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(EC.presence_of_element_located(tuple_selector))

    def wait_for_visibility_of_element_located(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(EC.visibility_of_element_located(tuple_selector))

    def wait_for_invisibility_of_element_located(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(EC.invisibility_of_element_located(tuple_selector))

    def wait_for_text_to_be_present_in_element(self, tuple_selector, text, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(EC.text_to_be_present_in_element(tuple_selector, text))

    def wait_for_element_to_be_clickable(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
    ...
    def wait_for_text_to_be_present_in_value_of_element(self, tuple_selector, text,
                                                        timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        """
    ...
    def wait_for_element_attribute_to_be(self, tuple_selector, attribute, value, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(ElementAttributeToBe(tuple_selector, attribute, value))

    def wait_for_css_property_to_be(self, tuple_selector, attribute, value, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(ElementCSSPropertyToBe(tuple_selector, attribute, value))

    def is_text_present_in_element(self, tuple_selector, text, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        try:
    ...
    # ---- check method ------------
    def is_element_selected(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        element = self.wait_for_visibility_of_element_located(tuple_selector, timeout)
        return element.is_selected()

    def is_element_visible(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        try:
    ...
    def is_element_invisible(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        try:
    ...
    def is_element_exist(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        try:
    ...
    def is_element_enabled(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        element = self.wait_for_visibility_of_element_located(tuple_selector, timeout)
    ...
    def click_and_select_link_text_from_menu_options(self, dropdown_locator, link_text,
                                                     timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS, move_to_element=False,
    ...
    def click_element(self, tuple_selector, move_to_element=False, move_to_element_by_script=False,
                      timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS, by_script=False):
        if move_to_element:
    ...
    def get_element_text(self, tuple_selector, move_to_element=False, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        if move_to_element:
            self.move_to_element(tuple_selector)
        element = self.wait_for_visibility_of_element_located(tuple_selector, timeout)
    ...
    def get_moved_elements_text(self, tuple_selector, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
    ...
    def get_text_of_elements(self, tuple_selector, move_to_element=False, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
        wait = WebDriverWait(self.driver, timeout)
        elements = wait.until(EC.presence_of_all_elements_located(tuple_selector))
    ...
    def get_attribute_of_element(self, tuple_selector, attribute, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS,
    ...
    def get_value_of_css_property(self, tuple_selector, attribute_name, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS,
                                  move_to_element=False):
        """
    ...
    def switch_to_frame(self, tuple_locator, timeout=EnvironmentConfiguration.SELENIUM_TIMEOUT_SECONDS):
